...
//                return;
  ...
  //[[#{text.initial.currency.drop.down}]]
        ...
        success: function (res) {
            ...
            $.each(msg.dataResult.data.records, function (i, v) {
                if(!v.hideFlag){
            		var id = v.id;
                    var text = v.coinName;
                    $("#payCoin").append("<option value='" + id + "'>" + text + "</option>");
                    $("#gotCoin").append("<option value='" + id + "'>" + text + "</option>");
            	}
            });
            form.render();
        },
    ...
    //debugger;
loadTableWithoutColumns('/tradingRecord/fastExchange/listPage', 'dataTable', 'searchForm');
...
function getStatusStr(status) {
	...
	if (status == 0 || status == 1 || status == 7) {
		...
		statusStr = "<font color='lightgreen'>[[#{text.partial.success}]]</font>";
...
function rowTool(){
    layui.use('table', function (){
        ...
        table.on('tool(dataTable)', function(obj){
            if(obj.event === 'detail'){
                detail("/tradingRecord/fastExchange/detail?id=" + obj.data.id);
            }
        });
    });
}
...
function download() {
	var url = "/tradingRecord/fastExchange/download/list?"+ $("#searchForm").serialize();
	// [[#{text.acquire.lock}]]
	var urlLock = "/tradingRecord/fastExchange/downloadLock?"+ $("#searchForm").serialize();
	$.ajaxUkey({
		url : urlLock,
		type : "post",
		async : "true",
