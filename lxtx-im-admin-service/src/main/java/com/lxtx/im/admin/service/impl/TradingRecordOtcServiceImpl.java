...
import com.lxtx.im.admin.service.response.TradingRecordOtcResp;
...
@Slf4j
@Service
public class TradingRecordOtcServiceImpl implements TradingRecordOtcService {

    private static final DateFormat DATEFORMAT = new SimpleDateFormat(DateUtils.DATE_FORMAT_DEFAULT);

    private static final String APPLY_LIST_FILENAME = "OTC充值交易流水列表";
    ...
    @Autowired
    private TradingRecordOtcFeign tradingRecordOtcFeign;
    ...
    @Override
    public BaseResult listPage(TradingRecordOtcReq req, HttpSession session) {
        ...
        FeignTradingRecordOtcReq feignTradingRecordOtcReq = new FeignTradingRecordOtcReq();
        ...
        BaseResult baseResult = tradingRecordOtcFeign.listPage(feignTradingRecordOtcReq);
        if (baseResult.isSuccess() && baseResult.getData() != null) {
            JSONObject jsonObject = JSONObject.parseObject(JSON.toJSONString(baseResult.getData()));
            List<TradingRecordOtcResp> respList = JSONObject.parseArray(jsonObject.getString("records"),
                    TradingRecordOtcResp.class);
    ...
    @Override
    public BaseResult downloadLock(TradingRecordOtcReq req, HttpSession session) {
        String userId = ShiroUtils.getUserId();
        String lockKey = RedisConstants.ADMIN_EXPORT_OTC_RECHARGE_LOCK.concat(userId);
    ...
    @Override
    public void downloadList(HttpServletResponse response, HttpSession session, TradingRecordOtcReq req) {
        String userId = ShiroUtils.getUserId();
        String lockKey = RedisConstants.ADMIN_EXPORT_OTC_RECHARGE_LOCK.concat(userId);
        String requestId = session.getId();
        try {
            ...
            List<TradingRecordOtcResp> respList = null;
            List<String> userIdListByImInfo = null;
            List<String> userIdsByAddr = null;
            ...
            if (StringUtils.isNotBlank(req.getAssignAddr())) {
                ...
                if (queryWalletUser.isSuccess() && queryWalletUser.getData() != null) {
                    JSONObject jsonObject = JSONObject.parseObject(JSON.toJSONString(queryWalletUser.getData()));
                    userIdsByAddr = JSONObject.parseArray(JSON.toJSONString(jsonObject.getJSONObject("data").getJSONArray("list")), String.class);
                    if (CollectionUtils.isEmpty(userIdsByAddr)) {
                        respList = Lists.newArrayList();
                        ;
                    }
                } else {
            ...
            BeanUtils.copyProperties(req, feignTradingRecordOtcReq);
            if (!CollectionUtils.isEmpty(userIdsByAddr)) {
                feignTradingRecordOtcReq.setUserIdsByAddr(userIdsByAddr);
            }
            if (!CollectionUtils.isEmpty(userIdListByImInfo)) {
                feignTradingRecordOtcReq.setUserIdsByUserName(userIdListByImInfo);
            }
    ...
    @Override
    public String detail(TradingRecordOtcDetailReq req, Model model) {
        FeignTradingRecordOtcDetailReq feignTradingRecordOtcDetailReq = new FeignTradingRecordOtcDetailReq();
        feignTradingRecordOtcDetailReq.setId(req.getId());
        BaseResult otcRechargeResult = tradingRecordOtcFeign.detail(feignTradingRecordOtcDetailReq);

        Map<String, Object> dataMap = (Map<String, Object>) otcRechargeResult.getData();
        String jsonResult = JSONArray.toJSONString(dataMap);
        TradingRecordOtcResp detailResp = JSONObject.parseObject(jsonResult, TradingRecordOtcResp.class);
        setState1Value(detailResp);
        model.addAttribute("detail", detailResp);
        return "wallet/tradingRecord-otc-detail";
    }

    private void setState1Value(TradingRecordOtcResp resp) {
        if (
                0 == resp.getState1()
                        || 1 == resp.getState1()
                        || 10 == resp.getState1()
